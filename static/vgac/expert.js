...
bClose.onclick = function() {
  ...
  drawinn.style.display = "none";
  draw_picture(
    canvas_drawinn.toDataURL(),
    canvas_list[canvas_drawinn.affordance_id],
    256,
    224
  );
...
function enlarge_aff() {
  is_big = 1;
  saveAffordanceImages();
  document.getElementById("name_big").textContent =
    "Current affordance is: " + this.getAttribute("name");
  sizex = 12;
  sizey = 12;
  drawinn.style.display = "block";
  draw_picture(
    output["tag_images"][
      Object.keys(output["tag_images"])[this.getAttribute("affordance_id")]
    ],
    canvas_drawinn,
    768,
    672
...
function draw_mouse_affordances(tmp, canvas, e) {
  ...
  if (mouseDown) {
    ...
    context.fillRect(
      pos.x - ((pos.x - grid_movex) % sizex),
      pos.y - ((pos.y - grid_movey) % sizey),
      sizex,
      sizey
    );
  }
...
for (var canvas_id = 0; canvas_id < canvas_list.length; canvas_id++) {
  ...
  window.addEventListener(
    "mousemove",
    draw_mouse_affordances.bind(null, event, canvas_list[canvas_id]),
    false
  );
  window.addEventListener(
    "mousedown",
  ...
  canvas_list[canvas_id].addEventListener("contextmenu", event =>
    event.preventDefault()
  ); //block rightclick on 'em
}
//----------------------------------------------------------------------------
...
window.addEventListener(
  "mousemove",
  draw_mouse_affordances.bind(null, event, canvas_drawinn),
  false
);
window.addEventListener(
  "mousedown",
...
window.addEventListener("load", function() {
  example_idx = 1;
  console.log("on load, example:", example_idx);
  fetch_data(example_idx);
});

function fetch_data(example) {
  console.log(mydata);
  if (example == 1) {
    temp_url = BASE_URL + "/test_data/expert1.json";
  } else if (example == 2) {
    temp_url = BASE_URL + "/test_data/expert2.json";
  } else {
    temp_url = BASE_URL + "/test_data/expert3.json";
  }
  $.getJSON(temp_url, function(json) {
    ...
    for (
      var index = 0;
      index < Object.keys(mydata["output"]["textures"]).length;
      index++
    ) {
...
function update_images() {
  ...
  document.getElementById("curr_textures").textContent =
    String(num) +
    "/" +
    String(Object.keys(mydata["output"]["textures"]).length) +
    " textures";
...
document.onkeydown = function(event) {
  ...
  case 8: //Backspace
    ...
    num = num - 1;
    if (num == -1) {
      //check evr time that num doesnt exceed amount of textures
      num = num + 1;
  ...
  case 13: //ENTER
    if (grid_checked) {
      ...
      num = num + 1;
      if (num == Object.keys(textures).length) {
        //check evr time that num doesnt exceed amount of textures
        num = num - 1;
    ...
    if (!is_big) {
      ...
      if (num == 0) {
        ...
        for (var canvas_id = 0; canvas_id < canvas_list.length; canvas_id++) {
          draw_picture(
            output["tag_images"][
              Object.keys(output["tag_images"])[canvas_id]
            ],
            canvas_list[canvas_id],
            256,
            224
